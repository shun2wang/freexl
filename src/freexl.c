...
/* setting a DATE value to some cell */
	...
	(row * workbook->active_sheet->maxcols) + col;
...
/* destroying a Sheet struct */
    ...
    if (sheet->cell_values)
	  ...
	  for (row = 0; row < sheet->rows; row++)
	    {
		/* destroying rows */
		p_cell = sheet->cell_values + (row * sheet->maxcols);
...
/* testing for an unrealistically high memory size > 256MB */
	workbook->active_sheet->maxcols = workbook->active_sheet->columns;
    dsize =
	(double) sizeof (biff_cell_value) *
	(double) (workbook->active_sheet->rows) *
	(double) (workbook->active_sheet->maxcols);
    if (dsize > 256.0 * 1024.0 * 1024.0)
	return FREEXL_INSUFFICIENT_MEMORY;

/* allocating the cell values array */
    if (workbook->active_sheet->rows * workbook->active_sheet->maxcols <= 0)
      {
    ...
    for (row = 0; row < workbook->active_sheet->rows; row++)
	  ...
	  p_cell =
	      workbook->active_sheet->cell_values +
	      (row * workbook->active_sheet->maxcols);
	  for (col = 0; col < workbook->active_sheet->maxcols; col++)
	    {
		p_cell->type = FREEXL_CELL_NULL;
		p_cell++;
	    }
 ...
 */
    ...
    if (workbook->ok_bof == 0)
	  ...
	  switch (workbook->record_type)
	    ...
	    default:
		workbook->ok_bof = 0;
/* 2023-08-14 patch submitted by Mark Finnis */
		return FREEXL_OK;
//      return FREEXL_BIFF_INVALID_BOF;
/* end Mark Finnis */
	    };
...
/* 2023-08-14 patch submitted by Mark Finnis */
    if (workbook->record_type == BIFF_BOF)
      {
	  /* We're not expecting it but we'll assume it's a new stream */
	  workbook->ok_bof = 1;
	  memcpy (word16.bytes, workbook->record + 2, 2);
	  if (swap)
	      swap16 (&word16);
...
/* 2023-08-14 patch submitted by Mark Finnis */
    if ((workbook->biff_content_type != 0x0005) &&
	(workbook->biff_content_type != 0x0010))
	return FREEXL_OK;
/* end Mark Finnis */
...
/* 2023-08-14 patch submitted by Mark Finnis */
	  if (workbook->record[5] == '\0')	// Ignore everything but Worksheets (and dialog sheets)
	    {
/* end Mark Finnis */
		if (!add_sheet_to_workbook
		    (workbook, offset.value, workbook->record[4],
		     workbook->record[5], utf8_name))
		    return FREEXL_INSUFFICIENT_MEMORY;
/* 2023-08-14 patch submitted by Mark Finnis */
	    }
/* end Mark Finnis */
	...
	&& (workbook->biff_version == FREEXL_BIFF_VER_5
	  ...
	  if (workbook->active_sheet != NULL)
		 ...
		 * sandro 2023-09-13
		 * 
		 * an extra Row and an extra Column are intentionally
		 * declared so to prevent possible errors caused by
		 * buggish writers that incorrectly report off-by-one
		 * values
		 */
		workbook->active_sheet->rows = rows + 1;
		workbook->active_sheet->columns = columns + 1;
		/* end sandro 2023-09-13 */
...
static void
freexl_xls_final_check(freexl_handle * handle)
{

/*
 * sandro 2023-09-13
 * 
 * it seems that several XLS writers incorrectly
 * declare off-by-one Rows and Columns
 * 
 * as a defensive measure in the previous steps
 * both Rows and Columns were incemented by +1
 * 		
* we'll now try to fix the correct values
*/
	biff_sheet *sheet;
	unsigned int row;
	unsigned short col;
    biff_workbook *workbook = handle->xls_handle;
	FreeXL_CellValue value;
	
	sheet = workbook->first_sheet;
	while (sheet != NULL)
	{
		/* testing each Workshet */
		int extra_row = 0;
		int extra_col = 0;

		for (row = 0; row < sheet->rows; row++)
		{
			/* looping on rows testing the last Column */
			col = sheet->columns - 1;
			if (freexl_get_cell_value (handle, row, col, &value) == FREEXL_OK)
			{
				if (value.type != FREEXL_CELL_NULL)
				extra_col = 1;	/* confirmed; there is an extra Column */
			}
		};
		
		for (col = 0; col < sheet->columns; col++)
		{
			/* looping on all COlumns of the last Row */
			row = sheet->rows - 1;
			if (freexl_get_cell_value (handle, row, col, &value) == FREEXL_OK)
			{
				if (value.type != FREEXL_CELL_NULL)
				extra_row = 1;	/* confirmed; there is an extra Row */
			}
		}

		if (extra_row == 0)
		{
			/* aremoving the extra Row */
			sheet->rows -=1;
		}
		if (extra_col == 0)
		{
...
static int
common_open_xls (const char *path, freexl_handle ** handle)
{
...
/* opening and initializing the Workbook - XLS format expected */
    ...
    return common_open_xls (path, handle);
}
...
/*
 * sandro 2023-09-13
 * simply an alias name for freexl_open()
 */
    return freexl_open (path, xl_handle);
}
		...
		if (p_col->assigned)
		  {
		      ...
		      if (p_col->is_datetime != XLSX_DATE_NONE)
			{
			    if (p_col->type == XLSX_STR_INDEX)
			      {
				  /* 2023-08-15 sandro
				   * this is not a DateTime, it's a Text String
				   */
		      ...
		      else
			{
			    ...
			    if (p_col->type == XLSX_STRING)
			      {
				  val->type = FREEXL_CELL_TEXT;
				  val->value.text_value = p_col->text;
			      }
			}
		      return FREEXL_OK;
		  }
...
/* attempting to fetch a cell value */
    ...
    p_cell =
	workbook->active_sheet->cell_values +
	(row * workbook->active_sheet->maxcols) + column;
/* 
